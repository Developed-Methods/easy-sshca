        ...
        match parts.next() {
            ...
            Some("sign") => {
                            ...
                            || key.eq_ignore_ascii_case("d")
                        {
                            ...
                            if duration.is_none() {
                                return Err(WebError(
                                    ...
                                    "unknown duration parameter options m(inute)/d(day)/h(our)/w(week)",
                                ));
                            }
                        }
                    }
                ...
                if !user.allowed_clients.iter().any(|c| c == &client.name) {
                    return Err(WebError(
                        StatusCode::UNAUTHORIZED,
                        "user not allowed to access through client",
                    ));
                }
                ...
                if !user.allowed_targets.iter().any(|t| t == &target.name) {
                    return Err(WebError(
                        StatusCode::UNAUTHORIZED,
                        "user not allowed to access target",
                    ));
                }
                ...
                let totp_secret = match tokio::fs::read_to_string(&totp_path).await {
                    Ok(v) => match TotpSecret::from_str(v.trim()) {
                        ...
                        Err(error) => {
                            ...
                            return Err(WebError(
                                StatusCode::INTERNAL_SERVER_ERROR,
                                "failed to parse totp secret on server",
                            ));
                        }
                    },
                    Err(error) => {
                ...
                let to_sign =
                    PublicKey::from_openssh(&load_body_str(request).await?).map_err(|_| {
                        WebError(
                            StatusCode::BAD_REQUEST,
                            "invalid openssh public key in request body",
                        )
                    })?;
...
fn parse_api_key<B>(req: &Request<B>) -> Result<String, WebError> {
    let api_key = req
        .headers()
        .get(http_app::header::AUTHORIZATION)
    ...
    if let Some(key) = api_key.strip_prefix("Api-Key: ") {
        ...
        Err(WebError(
            StatusCode::UNAUTHORIZED,
            "AUTHORIZATION header does start with \"Api-Key: \"",
        ))
    }
}
